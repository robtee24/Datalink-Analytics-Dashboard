...
export default function LinkedInAds({ dateRange, compareDateRange }: LinkedInAdsProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const result = await fetchAdsMetrics('linkedin', dateRange, compareDateRange);
        setData(result);
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <SectionHeader title="LinkedIn Ads" logoUrl="https://cdn.simpleicons.org/linkedin/0A66C2" />
