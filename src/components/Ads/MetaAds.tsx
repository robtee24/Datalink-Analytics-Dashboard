...
import { fetchMetaAds } from '../../services/metaAdsService';
...
export default function MetaAds({ dateRange, compareDateRange, loadTrigger }: MetaAdsProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const result = await fetchMetaAds(dateRange, compareDateRange);
        setData(result);
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <SectionHeader title="Meta Ads" logoUrl="https://cdn.simpleicons.org/meta/0081FB" />
